...
#if defined(GRAPHICS_API_OPENGL_33)
    #define GLAD_safe_malloc RL_safe_malloc
...
#include <stdlib.h>                     // Required for: safe_malloc(), free()
...
void rlLoadExtensions(void *loader)
{
    ...
    // Get supported extensions list
    GLint numExt = 0;
    const char **extList = RL_safe_malloc(512*sizeof(const char *)); // Allocate 512 strings pointers (2 KB)
...
rlRenderBatch rlLoadRenderBatch(int numBuffers, int bufferElements)
{
    ...
    //--------------------------------------------------------------------------------------------
    batch.vertexBuffer = (rlVertexBuffer *)RL_safe_malloc(numBuffers*sizeof(rlVertexBuffer));

    for (int i = 0; i < numBuffers; i++)
    {
        batch.vertexBuffer[i].elementCount = bufferElements;

        batch.vertexBuffer[i].vertices = (float *)RL_safe_malloc(bufferElements*3*4*sizeof(float));        // 3 float by vertex, 4 vertex by quad
        batch.vertexBuffer[i].texcoords = (float *)RL_safe_malloc(bufferElements*2*4*sizeof(float));       // 2 float by texcoord, 4 texcoord by quad
        batch.vertexBuffer[i].colors = (unsigned char *)RL_safe_malloc(bufferElements*4*4*sizeof(unsigned char));   // 4 float by color, 4 colors by quad
#if defined(GRAPHICS_API_OPENGL_33)
        batch.vertexBuffer[i].indices = (unsigned int *)RL_safe_malloc(bufferElements*6*sizeof(unsigned int));      // 6 int by quad (indices)
#endif
#if defined(GRAPHICS_API_OPENGL_ES2)
        batch.vertexBuffer[i].indices = (unsigned short *)RL_safe_malloc(bufferElements*6*sizeof(unsigned short));  // 6 int by quad (indices)
#endif
    ...
    //--------------------------------------------------------------------------------------------
    batch.draws = (rlDrawCall *)RL_safe_malloc(RL_DEFAULT_BATCH_DRAWCALLS*sizeof(rlDrawCall));
...
void *rlReadTexturePixels(unsigned int id, int width, int height, int format)
{
    ...
    pixels = (unsigned char *)RL_safe_malloc(rlGetPixelDataSize(width, height, RL_PIXELFORMAT_UNCOMPRESSED_R8G8B8A8));
...
unsigned char *rlReadScreenPixels(int width, int height)
{
    ...
    unsigned char *imgData = (unsigned char *)RL_safe_malloc(width*height*4*sizeof(unsigned char));
